...
describe("codename", () => {
  ...
  test("converts numbers consistently", () => {
    const result1 = codename(testWords, 1234);
    const result2 = codename(testWords, 1234);
  ...
  test("returns different names for different numbers", () => {
    ...
    for (let i = 0; i < 100; i++) {
      results.add(codename(testWords, i));
    }
    expect(results.size).toBeGreaterThan(1);
  });

  test("handles negative numbers", () => {
    const result = codename(testWords, -42);
    expect(testWords).toContain(result);
  });

  test("handles very large numbers", () => {
    const result = codename(testWords, Number.MAX_SAFE_INTEGER);
    expect(testWords).toContain(result);
  });

  test("handles decimal numbers", () => {
    const result = codename(testWords, 3.14159);
    expect(testWords).toContain(result);
  });

  test("throws error for non-finite numbers", () => {
    expect(() => codename(testWords, NaN)).toThrow(
      "Invalid input: expected a finite number, got NaN",
    );
    expect(() => codename(testWords, Infinity)).toThrow(
  ...
  test("distributes values evenly across word list", () => {
    ...
    for (let i = 0; i < iterations; i++) {
      const result = codename(testWords, i);

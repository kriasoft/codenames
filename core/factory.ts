 ...
 * @example
 * import { createCodename } from "codenames/core/factory";
...
export function createCodename(words: readonly string[]) {
  return (input: number): string => codename(words, input);
}
...
export function createTypedCodename<T extends readonly string[]>(words: T) {
  return (input: number): T[number] => codename(words, input) as T[number];
}
